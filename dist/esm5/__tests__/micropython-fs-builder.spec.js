...
import { addIntelHexFile, addIntelHexFiles, calculateFileSize, getIntelHexFiles, getIntelHexFsSize, } from '../micropython-fs-builder';
var uPyHexFile = fs.readFileSync('./src/__tests__/upy-v1.0.1.hex', 'utf8');
var makecodeHexFile = fs.readFileSync('./src/__tests__/makecode.hex', 'utf8');
var randContent = strToBytes('Some random content.');
describe('Writing files to the filesystem.', function () {
    var files = [
        {
        ...
        },
        {
            ...
            fileStr: '# Lorem Ipsum is simply dummy text of the printing and\r\n' +
    ...
    it('Add files to hex.', function () {
        var _a;
        var fwWithFsOther = addIntelHexFiles(uPyHexFile, (_a = {},
            _a[files[0].fileName] = strToBytes(files[0].fileStr),
            _a[files[1].fileName] = strToBytes(files[1].fileStr),
            _a));
        // fs.writeFileSync('./ignore/output2.hex', fwWithFsOther);
        // Address calculated starting at the top of the MicroPython v1.0.1 fs
        var opMap = MemoryMap.fromHex(fwWithFsOther);
        var startFs = 0x38c00;
        var file0data = opMap.slice(startFs, files[0].fsSize).get(startFs);
        var file1data = opMap
    ...
    var fullChunkPlus = {
        ...
        fileStr: 'a = """abcdefghijklmnopqrstuvwxyz\n' +
    ...
    it('Can generate a full chunk that also uses the next one.', function () {
        var fwWithFsOther = addIntelHexFile(uPyHexFile, fullChunkPlus.fileName, strToBytes(fullChunkPlus.fileStr));
        var opMap = MemoryMap.fromHex(fwWithFsOther);
        var readFileData = opMap
    ...
    it('Correctly generate an almost full chunk (not using last byte).', function () {
        var fwWithFsOther = addIntelHexFile(uPyHexFile, fullChunkMinus.fileName, strToBytes(fullChunkMinus.fileStr));
        var opMap = MemoryMap.fromHex(fwWithFsOther);
        var readFileData = opMap
    ...
    it('Correctlty generate just over a full chunk.', function () {
        var fwWithFsOther = addIntelHexFile(uPyHexFile, twoChunks.fileName, strToBytes(twoChunks.fileStr));
        var opMap = MemoryMap.fromHex(fwWithFsOther);
        var readFileData = opMap
    ...
    it('Empty file name throws an error.', function () {
        var failCase = function () { return addIntelHexFile(uPyHexFile, '', randContent); };
        expect(failCase).toThrow(Error);
    });
    it('Empty file data throw an error.', function () {
        var failCase = function () {
            var hexWithFs = addIntelHexFile(uPyHexFile, 'my_file.txt', new Uint8Array(0));
        };
        expect(failCase).toThrow(Error);
    });
    it('Large file that does not fit throws error.', function () {
        var failCase = function () {
            var hexWithFs = addIntelHexFile(uPyHexFile, 'my_file.txt', new Uint8Array(50 * 1024).fill(0x55));
        };
        expect(failCase).toThrow(Error);
    });
    it('Add files until no more fit.', function () {
        // The MicroPython hex has about 29 KBs
        var hexWithFs = uPyHexFile;
        // Use 4 KB blocks per file (each chunk is 128 B)
        var fakeBigFileData = new Uint8Array(4000).fill(0x55);
        var fakeSingleChunkData = new Uint8Array([0x55, 0x55]);
        var addLargeFiles = function () {
            // At 4Kbs we only fit 7 files
            for (var i = 0; i < 15; i++) {
                hexWithFs = addIntelHexFile(hexWithFs, 'file_' + i + '.txt', fakeBigFileData);
            }
        };
        var completeFsFilling = function () {
            // At a maximum of 4 Kbs left, it would fit 32 chunks max
            for (var i = 100; i < 132; i++) {
                hexWithFs = addIntelHexFile(hexWithFs, 'file_' + i + '.txt', fakeSingleChunkData);
            }
        };
        expect(addLargeFiles).toThrow(Error);
        expect(completeFsFilling).toThrow(Error);
    });
    it('Max filename length works.', function () {
        var maxLength = 120;
        var largeName = 'a'.repeat(maxLength);
        var workingCase = function () {
            return addIntelHexFile(uPyHexFile, largeName, randContent);
        };
    ...
    it('Too large filename throws error.', function () {
        ...
        var failCase = function () { return addIntelHexFile(uPyHexFile, largeName, randContent); };
        expect(failCase).toThrow(Error);
    });
    it('Adding files to non-MicroPython hex fails.', function () {
        var failCase = function () {
            return addIntelHexFile(makecodeHexFile, 'a.py', randContent);
        };
        expect(failCase).toThrow(Error);
    });
...
describe('Reading files from the filesystem.', function () {
    var alastFilename = 'alast.py';
    var alastContent = strToBytes('# Lorem Ipsum is simply dummy text of the printing and\n' +
        "# typesetting industry. Lorem Ipsum has been the industry's\n" +
        '# standard dummy text ever since the 1500s, when an unknown\n' +
        '# printer took a galley of type and scrambled it to make a\n' +
        '# type specimen book. It has survived not only five\n' +
    ...
    var alastHex = ':020000040003F7\n' +
        ...
        ':00000001FF\n';
    var afirstFilename = 'afirst.py';
    var afirstContent = strToBytes("firstname = 'Carlos'");
    // Uses chunk ??
    var afirstHex = ':020000040003F7\n' +
        ':10DF0000FE1F096166697273742E70796669727397\n' +
        ':10DF1000746E616D65203D20274361726C6F7327BD\n' +
        ':00000001FF\n';
    var mainFilename = 'main.py';
    var mainContent = strToBytes('from microbit import display, Image, sleep\n\n' +
    ...
    var mainHex = ':020000040003F7\n' +
        ...
        ':00000001FF\n';
    it('Can read files of different sizes in non-consecutive locations.', function () {
        var addHexToMap = function (hexMap, hex) {
            var newMemMap = MemoryMap.fromHex(hex);
            newMemMap.forEach(function (value, index) {
        ...
        var foundFiles = getIntelHexFiles(fullUpyFsMemMap.asHexString());
    ...
    var oneChunkPlusFilename = 'one_chunk_plus.py';
    var oneChunkPlusContent = 'a = """abcdefghijklmnopqrstuvwxyz\n' +
        'abcdefghijklmnopqrstuvwxyz\n' +
        'abcdefghijklmnopqrstuvwxyz\n' +
        'abcdefghijklmno"""\n';
    // Uses chunk 0x45 and 0x46
    var oneChunkPlusHex = ':020000040003F7\n' +
        ...
        ':00000001FF\n';
    it('Can read a file that occupies a single chunk and also the next.', function () {
        ...
        var fullUpyFsMemMap = MemoryMap.fromHex(uPyHexFile);
        var oneChunkPlusMemMap = MemoryMap.fromHex(oneChunkPlusHex);
        oneChunkPlusMemMap.forEach(function (value, index) {
            fullUpyFsMemMap.set(index, value);
        });
        var foundFiles = getIntelHexFiles(fullUpyFsMemMap.asHexString());
        expect(foundFiles).toHaveProperty([oneChunkPlusFilename], strToBytes(oneChunkPlusContent));
    });
    var oneChunkMinusFilename = 'one_chunk_minus.py';
    var oneChunkMinusContent = 'a = """abcdefghijklmnopqrstuvwxyz\n' +
        'abcdefghijklmnopqrstuvwxyz\n' +
        'abcdefghijklmnopqrstuvwxyz\n' +
        'abcdefghijklm"""\n';
    // Uses chunk ??
    var oneChunkMinusHex = ':020000040003F7\n' +
        ...
        ':00000001FF\n';
    it('Can read a file that occupies almost a full chunk.', function () {
        ...
        var fullUpyFsMemMap = MemoryMap.fromHex(uPyHexFile);
        var oneChunkMinusMemMap = MemoryMap.fromHex(oneChunkMinusHex);
        oneChunkMinusMemMap.forEach(function (value, index) {
            fullUpyFsMemMap.set(index, value);
        });
        var foundFiles = getIntelHexFiles(fullUpyFsMemMap.asHexString());
        expect(foundFiles).toHaveProperty([oneChunkMinusFilename], strToBytes(oneChunkMinusContent));
    });
    var duplicateFilename = 'a.py';
    // Uses chunks ??
    var oneFileCopyHex = ':020000040003F7\n' +
        ':10AE0000FE1804612E707961203D20274A75737405\n' +
        ':10AE100020612066696C65270AFFFFFFFFFFFFFFC7\n' +
        ':00000001FF\n';
    var otherFileCopyHex = ':020000040003F7\n' +
        ':10C18000FE1804612E707961203D20274A75737472\n' +
        ':10C1900020612066696C65270AFFFFFFFFFFFFFF34\n' +
        ':00000001FF\n';
    it('Duplicate file names throws an error.', function () {
        // In contrast to the one_chunk_plus.py example, this one fills the chunk
        // minus 1 Byte (The second 0xFF at the end is the chunk tail).
        var fullUpyFsMemMap = MemoryMap.fromHex(uPyHexFile);
        var oneFileCopyMemMap = MemoryMap.fromHex(oneFileCopyHex);
        oneFileCopyMemMap.forEach(function (value, index) {
            fullUpyFsMemMap.set(index, value);
        });
        var otherFileCopyMemMap = MemoryMap.fromHex(otherFileCopyHex);
        otherFileCopyMemMap.forEach(function (value, index) {
            fullUpyFsMemMap.set(index, value);
        });
        var failCase = function () { return getIntelHexFiles(fullUpyFsMemMap.asHexString()); };
        expect(failCase).toThrow(Error);
    });
    it('Reading files from empty MicroPython hex returns empty list.', function () {
        var foundFiles = getIntelHexFiles(uPyHexFile);
        expect(foundFiles).toEqual({});
    });
    it('Reading files from non-MicroPython hex fails.', function () {
        var failCase = function () { return getIntelHexFiles(makecodeHexFile); };
        expect(failCase).toThrow(Error);
    });
...
describe('Calculate sizes.', function () {
    it('Get how much available fs space there is in a MicroPython hex file.', function () {
        var totalSize = getIntelHexFsSize(uPyHexFile);
        // Calculated by hand from the uPyHexFile v1.0.1 release.
        expect(totalSize).toEqual(27 * 1024);
    });
    it('Calculate the space occupied for a file in the fs.', function () {
        var fileSizeOne = calculateFileSize('one_chunk.txt', new Uint8Array([30, 31, 32, 33, 34]));
        var fileSizeAlmostFullOne = calculateFileSize('almost_one_chunk____.txt', new Uint8Array(99).fill(0x35));
        var fileSizeOneOverflow = calculateFileSize('one_chunk_overflow__.txt', new Uint8Array(100).fill(0x35));
        var fileSizeTwo = calculateFileSize('just_about_2_chunks_.txt', new Uint8Array(101).fill(0x35));
        var fileSizeNine = calculateFileSize('9_chunks.txt', new Uint8Array(1100).fill(0x35));
...
//# sourceMappingURL=micropython-fs-builder.spec.js.map