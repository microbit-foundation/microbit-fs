...
import { bytesToStr, cleanseOldHexFormat, concatUint8Array, strToBytes, } from './common';
import { isAppendedScriptPresent } from './micropython-appended';
...
function getFreeChunks(intelHexMap) {
    ...
    while (chunkAddr < endAddress) {
        var marker = intelHexMap.slicePad(chunkAddr, 1, 255 /* Unused */)[0];
...
function getStartAddress(intelHexMap) {
    ...
    var startAddress = uicrData.runtimeEndAddress;
...
function setPersistentPage(intelHexMap) {
    ...
    intelHexMap.set(getLastPageAddress(intelHexMap), new Uint8Array([253 /* PersistentData */]));
}
/**
...
function addMemMapFile(intelHexMap, filename, data) {
    ...
    var chunksStartAddress = chuckIndexAddress(intelHexMap, freeChunks[0]);
...
function getIntelHexFiles(intelHex) {
    ...
    var endAddress = getLastPageAddress(hexMap);
    ...
    var chunkIndex = 1;
    while (chunkAddr < endAddress) {
        var chunk = hexMap.slicePad(chunkAddr, CHUNK_LEN, 255 /* Unused */);
        var marker = chunk[0];
        if (marker !== 255 /* Unused */ &&
            marker !== 0 /* Freed */ &&
            marker !== 253 /* PersistentData */) {
    ...
    for (var _i = 0, startChunkIndexes_1 = startChunkIndexes; _i < startChunkIndexes_1.length; _i++) {
        ...
        var endChunkOffset = startChunk[1 /* EndOffset */];
        var filenameLen = startChunk[2 /* NameLength */];
        ...
        var currentIndex = startChunkIndex;
        ...
        while (iterations--) {
            var nextIndex = currentChunk[127 /* Tail */];
            if (nextIndex === 255 /* Unused */) {
                ...
                files[filename] = concatUint8Array(files[filename], currentChunk.slice(chunkDataStart, 127 /* Tail */));
            }
...
function getIntelHexFsSize(intelHex) {
    ...
    var intelHexMap = MemoryMap.fromHex(intelHexClean);
    var startAddress = getStartAddress(intelHexMap);
    var endAddress = getEndAddress(intelHexMap);
